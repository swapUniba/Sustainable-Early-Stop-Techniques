...
class Trainer(AbstractTrainer):
    ...
    def fit(
        ...
        Args:
            ...
            trade_off (float) --- smoothing_factor (float)
        ...
        # var
        valid_step = 0
        emission_step = 0 # emission_step = 0 (con trade_off) || emission_step = max_emission_step (con smoothing)
        last_emissions = None
        last_utility = None
        self.best_valid_score = 0

        with OfflineEmissionsTracker(tracking_mode='process', on_csv_write='append', output_file=emission_file_path, project_name=proj_name, country_iso_code="WRD") as tracker:
            for epoch_idx in range(self.start_epoch, self.epochs):
                ...
                if emission_flag:
                    stop_output = "Finished training, max emissions step reached in epoch %d" % (epoch_idx)
                    if verbose:
                        self.logger.info(stop_output)
                    break
