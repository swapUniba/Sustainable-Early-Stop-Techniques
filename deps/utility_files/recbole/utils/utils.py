...
"""def early_stopping(value, best, cur_step, max_step, emission_step, max_emission_step, total_emissions, last_emissions, smoothing_factor, last_utility, bigger=True):
    stop_flag = False
    update_flag = False
    emission_flag = False
    if bigger:
        if value >= best:
            ...
            if last_emissions is not None:
                # new_ratio sarebbe rappresentato da utility || ema_ratio rapp da last_utility (non ho cambiato var per far casini se no)
                utility = (value-best)/(total_emissions-last_emissions)
                if last_utility is not None:
                    if last_utility > utility:
                        emission_step = max(0, emission_step-1)
                    else:
                        emission_step += 0.5 # = min(max_emission_step, emission_step+0.5) \\ emission_step += 0.5
                    if emission_step <= 0:
                        emission_flag = True
                last_utility = ema(utility, last_utility, smoothing_factor)
        ...
        if value <= best:
            ...
            if cur_step > max_step:
                stop_flag = True
    return best, cur_step, stop_flag, update_flag, emission_flag, emission_step, last_emissions, last_utility"""

def early_stopping(value, best, cur_step, max_step, emission_step, max_emission_step, total_emissions, last_emissions, trade_off, last_utility, bigger=True):
    stop_flag = False
    update_flag = False
    emission_flag = False
    if bigger:
        if value >= best:
            ...
            update_flag = True
            if last_emissions is not None:
                utility = (value-best) - ((total_emissions-last_emissions) * trade_off)
                if utility >= 0:
                    emission_step = 0
                else:
                    #applico seconda chance
                    if last_utility is not None and (last_utility + utility) < 0:
                        emission_step += 1
